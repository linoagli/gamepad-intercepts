...
#include "CGPICredentialProvider.h"
#include "CGPICredential.h"
#include "clsids.h"

HRESULT CGPI_CreateInstance(_In_ REFIID riid, _Outptr_ void** ppv)
{
	HRESULT hr;
	CGPICredentialProvider* pProvider = new(std::nothrow) CGPICredentialProvider();
...
CGPICredentialProvider::CGPICredentialProvider() :
	_cRef(1),
	...
	_pCredProviderUserArray(nullptr)
{
	...
	HRESULT hr = _pGPIAuthModulePtr.CreateInstance(__uuidof(GamePad_Intercepts_Authentication_Module::GamePadInterceptsAuthenticationModule));
	
	if (SUCCEEDED(hr))
	{
		_pGPIAuthModulePtr->StartControllerManagerService();
	}
	else
	{
		_pGPIAuthModulePtr = nullptr;
	}
}

CGPICredentialProvider::~CGPICredentialProvider()
{
	if (_pGPIAuthModulePtr != nullptr)
	{
		_pGPIAuthModulePtr->StopControllerManagerService();
		_pGPIAuthModulePtr->Release();
		_pGPIAuthModulePtr = nullptr;
	}
...
HRESULT CGPICredentialProvider::SetUsageScenario(CREDENTIAL_PROVIDER_USAGE_SCENARIO cpus, DWORD dwFlags)
{
	HRESULT hr;
...
// into this sample.]
HRESULT CGPICredentialProvider::SetSerialization(
...
HRESULT CGPICredentialProvider::Advise(
...
HRESULT CGPICredentialProvider::UnAdvise()
{
	return E_NOTIMPL;
}
...
HRESULT CGPICredentialProvider::GetFieldDescriptorCount(
	_Out_ DWORD* pdwCount)
{
	*pdwCount = GPI_FIELD_ID_NUM_FIELDS;
	return S_OK;
}

// Gets the field descriptor for a particular field.
HRESULT CGPICredentialProvider::GetFieldDescriptorAt(
	DWORD dwIndex,
	_Outptr_result_nullonfailure_ CREDENTIAL_PROVIDER_FIELD_DESCRIPTOR** ppcpfd)
{
	...
	*ppcpfd = nullptr;
	...
	if ((dwIndex < GPI_FIELD_ID_NUM_FIELDS) && ppcpfd)
	{
...
HRESULT CGPICredentialProvider::GetCredentialCount(
	_Out_ DWORD* pdwCount,
...
// the tiles.
HRESULT CGPICredentialProvider::GetCredentialAt(
	DWORD dwIndex,
...
HRESULT CGPICredentialProvider::SetUserArray(_In_ ICredentialProviderUserArray* users)
{
...
bool CGPICredentialProvider::IsGPIAuthModuleReady()
{
	return _pGPIAuthModulePtr != nullptr;
}

GamePad_Intercepts_Authentication_Module::IGamePadInterceptsAuthenticationModulePtr CGPICredentialProvider::GetGPIAuthModulePtr()
{
	return _pGPIAuthModulePtr;
}

void CGPICredentialProvider::_ReleaseEnumeratedCredentials()
{
	if (_pCredential != nullptr)
	{
		_pCredential->Release();
...
void CGPICredentialProvider::_CreateEnumeratedCredentials()
{
	switch (_cpus)
	{
	case CPUS_LOGON:
...
HRESULT CGPICredentialProvider::_EnumerateCredentials()
{
	...
	if (_pCredProviderUserArray != nullptr)
	{
		...
		if (dwUserCount > 0)
		{
			...
			if (SUCCEEDED(hr))
			{
				_pCredential = new(std::nothrow) CGPICredential();
				if (_pCredential != nullptr)
				{
